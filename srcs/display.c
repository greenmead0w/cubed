...
static	int	is_border_map(int curr_x, int curr_y, int start_x,
		int start_y)
{
	int	is_vert_border;
	int	is_horz_border;

	is_vert_border = (curr_x == start_x || curr_x == start_x + MINI_TILE -1);
	is_horz_border = (curr_y == start_y || curr_y == start_y + MINI_TILE -1);
	return (is_vert_border || is_horz_border);
}

static	void	draw_row(int y, int start_x, char value,
	t_game *game, int start_y)
{
	int	curr_x;
	int	remaining;
...
*/
static void	mapping_texture_pixels(t_vars *vars, t_ray ray, t_conn *conn,
	double wh_pos, int i)
{
	double			step;
	double			wall_offset;
...
void	draw_ray_cast(t_game *game, int i)
{
	...
	while (i < game->vars->num_rays)
	{
		...
		mapping_texture_pixels(game->vars, game->rays[i], game->conn,
			wall_hit_pos, 0);
		i++;
	}
}
